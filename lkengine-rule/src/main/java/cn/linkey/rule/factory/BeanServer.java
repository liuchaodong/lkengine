...
final public class BeanServer {
	...
	public static Object getBeanFromTable(String ruleNum,String tableName){
		...
		try{
			...
			HashMap<String, String> map = getRemoteResultSet(ruleNum,tableName);
			if(!map.isEmpty()){
				for(Map.Entry<String, String> entry : map.entrySet()){
					return getBeanByCode( entry.getValue(),entry.getKey());
				}
			}
			else if(rs == null){
				sql = "select rulecode,classpath from "+ (StringUtils.isNotBlank(tableName)?tableName:"bpm_rulelist") +" WHERE rulenum ='"+ruleNum+"' limit 1";
	 ...
	 */
    public static HashMap getRemoteResultSet(String ruleNum,String tableName){

		return BeanCtx.getRdbBean().getRuleCodeMapByRuleNum(ruleNum,tableName);
	}

	public static void main(String[] args) throws Exception {
		LinkeyRule insLinkeyRule = (LinkeyRule)BeanServer.getBeanByRuleNum("TestImpl","");
		System.out.println(insLinkeyRule.run(new HashMap<String, Object>()));
	}
}
