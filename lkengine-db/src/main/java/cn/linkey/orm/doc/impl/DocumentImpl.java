...
public class DocumentImpl implements Document,Serializable {
   ...
   /* private Rdb rdb = new RdbImpl();*/
    ...
    public String toJson(boolean useTableConfig) {
        if (useTableConfig) {
            ...
            String fieldConfig = BeanCtx.getRdbBean().getValueBySql(sql);
   ...
   /* @Override
    ...
    public void initByDocUnid(String docUnid) {
        ...
        BeanCtx.getRdbBean().appendDataFromSql(this, sql);
    }
    ...
    public String getLockStatus(String userid) {
        ...
        String lockUserid = BeanCtx.getRdbBean().getValueBySql(sql);
    ...
    public boolean unlock() {
        int i = BeanCtx.getRdbBean().execSql("delete from BPM_LockDocList where DocUnid='" + this.getDocUnid() + "'");
        if (i > 0) {
            return true;
        }
        else {
    ...
    public void setTableName(Connection conn, String tableName) {
        ...
        this.dbType = BeanCtx.getRdbBean().getDbType(conn);
    }
    ...
    public String getDocUnid() {
        String docunid = g("WF_OrUnid");
        if (Tools.isBlank(docunid)) {
            docunid = BeanCtx.getRdbBean().getNewUnid();
        }
        return BeanCtx.getRdbBean().formatArg(docunid);
    }
    ...
    public Document copyTo(Connection conn, String tableName) {
        ...
        newdoc.s("WF_OrUnid", BeanCtx.getRdbBean().getNewUnid());
        newdoc.save(conn);
        return newdoc;
    }
    ...
    public Document copyAllItems(Document targetDocument) {
        ((DocumentImpl)targetDocument).fdmap.putAll(this.fdmap);
    ...
    public HashMap<String, String> getColumnList(Connection conn) {
        ...
        if (this.tableFdConfig == null) {
            ...
            if (conn == null) {
                this.dbType = BeanCtx.getRdbBean().getDbType(); //获得数据库链接类型
                /*if (tableName.startsWith("BPM_") && BeanCtx.getSystemConfig("CacheSystemTableConfig").equals("1")) {
                    ...
                    //不进行缓存
                    this.tableFdConfig = BeanCtx.getRdbBean().getTableColumnName(tableName);
                //}
            }
            else {
                //不是默认链接的情况下不进行数据库表的缓存
                this.dbType = BeanCtx.getRdbBean().getDbType(conn); //获得数据库链接类型
                this.tableFdConfig = BeanCtx.getRdbBean().getTableColumnName(conn, tableName);
            }
        }
        return this.tableFdConfig;
    }
    ...
    public int save(Connection conn, String tableName, String extendTableName) {
        ...
        if (conn == null) {
            if (this.tableFdConfig == null || this.tableFdConfig.size() == 0) {
                ...
                this.tableFdConfig = BeanCtx.getRdbBean().getTableColumnName(conn, tableName); //重新计算
            }
        }
        else {
            //如果是新的链接对像则要重新计算链接对像的数据库类型和字段配置类型
            this.setDbType(BeanCtx.getRdbBean().getDbType(conn)); //设置为oracle数据库类型
            this.tableFdConfig = BeanCtx.getRdbBean().getTableColumnName(conn, tableName);//重新计算
        }
    ...
    public Object getBean(String calssName) {
        Object obj = null;
        //calssName = "cn.linkey.orm.doc.impl."+calssName;
    ...
    public int remove(Connection conn, boolean trash) {
        ...
        if (conn == null) {
            try {
                conn = BeanCtx.getRdbBean().getConnection();
            }
            catch (Exception e) {
                System.out.println("Doc.removeToTrash()获得数据库链接出错!");
            }
        }
        int i = 2;
        if (trash) {
        ...
        return BeanCtx.getRdbBean().execSql(conn, sql);
    }
