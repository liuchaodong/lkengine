...
public class RdbImpl implements Rdb {
    ...
    public  Connection getConnection() throws Exception {
        ...
        return Tools.isBlank(dbDriver)?BeanCtx.getConnection():BeanCtx.getConnection(dbDriver,dbUser,dbPwd,dbUrl);
    }
    ...
    public  Document[] getAllDocumentsBySql(Connection conn, String tableName, String sql, int pageNum, int pageSize) {
        ...
        try {
            ...
            while (rs.next()) {
                Document doc = BeanCtx.getDocumentBean(conn,tableName);
    ...
    public  LinkedHashSet<Document> getAllDocumentsSetByPage(Connection conn, String tableName, String fieldList, String orderByStr, String whereSql, int pageNum, int pageSize) {
        ...
        try {
            ...
            rs = getResultSet(conn, sql);
            while (rs.next()) {
                    Document doc = BeanCtx.getDocumentBean(conn,"");
                doc.appendFromResultSet(rs);
                docCollection.add(doc);
            }
        }
    ...
    @Override
    public HashMap<String, String> getRuleCodeMapByRuleNum(String ruleNum,String tableName) {
        String sql = "select classPath,ruleCode from "+(Tools.isNotBlank(tableName)?tableName:"bpm_rulelist")+ " where ruleNum='"+ruleNum+"' limit 1";
        return this.getMapDataBySql(sql);
    }
